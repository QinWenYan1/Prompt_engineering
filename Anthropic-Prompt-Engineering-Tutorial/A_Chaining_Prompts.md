...
- 我们还可以要求 Claude 在之前的回复上改进出下一个更好的版本
- 这个方法适用于任何需要迭代改进的内容
...
**替换占位符的方式十分有效，还可以用于 function calling**
- **函数调用**的事情——就是让 Claude 执行某个功能，然后获取该功能的结果，再让 Claude 用这些结果去做更多的事情。它的运作方式和其他任何替换一样。更多内容将在下一个附录中介绍。

- 一个两步骤的提示链，展示了函数调用(`Function Calling`)模式：
- 第1步从对话中提取所有人名，XML 标签输出：
